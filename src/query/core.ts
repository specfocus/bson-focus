...
export interface Operation<TItem> {
  ...
  reset(): void;
  next(item: TItem, key?: Key, owner?: any): void;
}
...
abstract class BaseOperation<TParams, TItem = any> implements Operation<TItem> {
  ...
  abstract next(item: any, key: Key, parent: any): void;
}
...
abstract class GroupOperation extends BaseOperation<any> {
  ...
  abstract next(item: any, key: Key, owner: any): void;
...
export class NestedOperation extends GroupOperation {
  ...
  private _nextNestedValue = (value: any, key: Key, owner: any): boolean => {
    this.childrenNext(value, key, owner);
    return !this.done;
  };
}

export const createTester = (a: any, compare: Comparator) => {
  ...
  return (b: any) => compare(comparableA, comparable(b));
};
...
export const numericalOperation = (createTester: (val: any) => Tester) =>
...
const createQueryOperations = (
  query: any,
  options: Options
): [Operation<any>[], NestedOperation[]] => {
  const selfOperations: Operation<any>[] = [];
...
export const createOperationTester =
  <TItem>(operation: Operation<TItem>): Tester =>
